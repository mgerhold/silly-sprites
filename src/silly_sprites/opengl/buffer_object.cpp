...
namespace sly::gl {
    ...
    void BufferObject::data_vertex_buffer() const {
        glBufferData(GL_ARRAY_BUFFER, sizeof(float) * m_points.size(), m_points.data(), GL_STATIC_DRAW);
    }

    void BufferObject::data_element_buffer() const {
        glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(usize) * m_indices.size(), m_indices.data(), GL_STATIC_DRAW);
    }
